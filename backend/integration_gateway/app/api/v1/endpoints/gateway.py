...
@router.post("/gigachat/chat")
async def gigachat_chat_proxy(request: Request):
  """Proxy GigaChat chat requests to core-agent-service."""
  try:
    body = await request.body()

    async with httpx.AsyncClient() as client:
      response = await client.post(
        f"{settings.CORE_AGENT_URL}/api/v1/gigachat/chat",
        content=body,
        headers=dict(request.headers),
        timeout=60.0,
      )
    ...
    logger.error("Timeout proxying GigaChat chat request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service timeout",
      details={"service": "core-agent-service", "error": str(e)},
    )
  except httpx.ConnectError as e:
    logger.error("Connection error proxying GigaChat chat request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service unavailable",
      details={"service": "core-agent-service", "error": str(e)},
    )
    ...
    logger.error(
      "HTTP error proxying GigaChat chat request",
      status_code=e.response.status_code,
      error=str(e),
    )
    raise ProxyError(
    ...
    logger.error("Unexpected error proxying GigaChat chat request", error=str(e), exc_info=True)
    raise ProxyError(
      "Failed to proxy request",
      details={"service": "core-agent-service", "error": str(e)},
    )
...
@router.get("/gigachat/context-info")
async def gigachat_context_info_proxy(request: Request):
  """Proxy GigaChat context info to core-agent-service."""
  try:
    async with httpx.AsyncClient() as client:
      response = await client.get(
        f"{settings.CORE_AGENT_URL}/api/v1/gigachat/chat/context-info",
        headers=dict(request.headers),
        timeout=10.0,
      )
    ...
    logger.error("Timeout proxying GigaChat context-info request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service timeout",
      details={"service": "core-agent-service", "error": str(e)},
    )
  except httpx.ConnectError as e:
    logger.error("Connection error proxying GigaChat context-info request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service unavailable",
      details={"service": "core-agent-service", "error": str(e)},
    )
    ...
    logger.error(
      "HTTP error proxying GigaChat context-info request",
      status_code=e.response.status_code,
      error=str(e),
    )
    raise ProxyError(
    ...
    logger.error("Unexpected error proxying GigaChat context-info request", error=str(e), exc_info=True)
    raise ProxyError(
      "Failed to proxy request",
      details={"service": "core-agent-service", "error": str(e)},
    )
...
@router.post("/gigachat/chat/{session_id}/compress")
async def gigachat_compress_proxy(session_id: str, request: Request):
  """Proxy GigaChat compress requests to core-agent-service."""
  try:
    body = await request.body()

    async with httpx.AsyncClient() as client:
      response = await client.post(
        f"{settings.CORE_AGENT_URL}/api/v1/gigachat/chat/{session_id}/compress",
        content=body,
        headers=dict(request.headers),
        timeout=60.0,
      )
    ...
    logger.error("Timeout proxying GigaChat compress request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service timeout",
      details={"service": "core-agent-service", "error": str(e)},
    )
  except httpx.ConnectError as e:
    logger.error("Connection error proxying GigaChat compress request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service unavailable",
      details={"service": "core-agent-service", "error": str(e)},
    )
    ...
    logger.error(
      "HTTP error proxying GigaChat compress request",
      status_code=e.response.status_code,
      error=str(e),
    )
    raise ProxyError(
    ...
    logger.error("Unexpected error proxying GigaChat compress request", error=str(e), exc_info=True)
    raise ProxyError(
      "Failed to proxy request",
      details={"service": "core-agent-service", "error": str(e)},
    )
...
@router.get("/gigachat/chat/{session_id}/memory")
async def gigachat_memory_proxy(session_id: str, request: Request):
  """Proxy GigaChat memory requests to core-agent-service."""
  try:
    async with httpx.AsyncClient() as client:
      response = await client.get(
        f"{settings.CORE_AGENT_URL}/api/v1/gigachat/chat/{session_id}/memory",
        headers=dict(request.headers),
        timeout=30.0,
      )
    ...
    logger.error("Timeout proxying GigaChat memory request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service timeout",
      details={"service": "core-agent-service", "error": str(e)},
    )
  except httpx.ConnectError as e:
    logger.error("Connection error proxying GigaChat memory request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service unavailable",
      details={"service": "core-agent-service", "error": str(e)},
    )
    ...
    logger.error(
      "HTTP error proxying GigaChat memory request",
      status_code=e.response.status_code,
      error=str(e),
    )
    raise ProxyError(
    ...
    logger.error("Unexpected error proxying GigaChat memory request", error=str(e), exc_info=True)
    raise ProxyError(
      "Failed to proxy request",
      details={"service": "core-agent-service", "error": str(e)},
    )
...
@router.delete("/gigachat/chat/{session_id}")
async def gigachat_clear_proxy(session_id: str, request: Request):
  """Proxy GigaChat clear session requests to core-agent-service."""
  try:
    async with httpx.AsyncClient() as client:
      response = await client.delete(
        f"{settings.CORE_AGENT_URL}/api/v1/gigachat/chat/{session_id}",
        headers=dict(request.headers),
        timeout=30.0,
      )
    ...
    logger.error("Timeout proxying GigaChat clear request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service timeout",
      details={"service": "core-agent-service", "error": str(e)},
    )
  except httpx.ConnectError as e:
    logger.error("Connection error proxying GigaChat clear request", error=str(e))
    raise ServiceUnavailableError(
      "Backend service unavailable",
      details={"service": "core-agent-service", "error": str(e)},
    )
    ...
    logger.error(
      "HTTP error proxying GigaChat clear request",
      status_code=e.response.status_code,
      error=str(e),
    )
    raise ProxyError(
    ...
    logger.error("Unexpected error proxying GigaChat clear request", error=str(e), exc_info=True)
    raise ProxyError(
      "Failed to proxy request",
      details={"service": "core-agent-service", "error": str(e)},
    )
